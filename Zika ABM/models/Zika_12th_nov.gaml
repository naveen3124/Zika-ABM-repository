...
//exponential
	...
	file shape_file_buildings <- file("../includes/ShpFiles/Delhi/Shivajiroad/Shivajiroad.shp");
	file shape_file_roads <- file("../includes/ShpFiles/Delhi/Shivajiroad/Shivajiroadr.shp");
	file shape_file_bounds <- file("../includes/ShpFiles/Delhi/Shivajiroad/Shivajiroadb.shp");
...
//	https://www.yr.no/place/India/Karnataka/Bangalore/statistics.html
	reflex temperature_update when: time mod 86400 = 0{
 	...
 	//	if(days_passed mod 30 = 1){write "rain probability" + rain_probabilty;}
 		int val <- 0 + rnd (2) ;
 	...
 	//	write "hello" + prec_rainfall + "temp" + cur_temp ;
 		...
 		int eggcount <-leftmosquitoes * prob_mating * sex_ratio;
 		if(prec_rainfall >=2 and prec_rainfall <= 7)
		{
			...
			create egg  number:(eggcount) {
				living_place <- one_of(watersources);
				location <- any_location_in(living_place);	
				age <- 0;
				if flip(0.05) {is_infected <- true;}
			}
	...
	reflex save_result when: (((days_passed = 360 or days_passed = 60 or days_passed = 180 or days_passed = 240) and current_hour = 2))  {
	/* 	save ("cycle: "+ cycle + "; nb_people_infected: " + nb_people_infected
			...
			+ "; nb_microcephaly_cases: " + nb_microcephaly_cases
	   		...
	   		* 
	   		*/
	   		write "naveen" + days_passed ;
	   	if(days_passed = 60)
	   	{	
	   		save (" " + nb_infected_cumulative + ", " + days_passed + ", " + nb_microcephaly_cases
	   		+ ", " + nb_home_place_infections           
	   		+ ", " + nb_working_place_infections       
	   		+ ", " + nb_leisure_place_infections) 
	   		to: "captured_data60.csv" type: "csv" ;
	   	...
	   	{	
	   		...
	   		+ ", " + nb_working_place_infections       
	   		+ ", " + nb_leisure_place_infections) 
	   		to: "captured_data180.csv" type: "csv" ;
	   	...
	   	{	
	   		...
	   		+ ", " + nb_working_place_infections       
	   		+ ", " + nb_leisure_place_infections) 
	   		to: "captured_data240.csv" type: "csv" ;
	   	...
	   	{	
	   		...
	   		+ ", " + nb_working_place_infections       
	   		+ ", " + nb_leisure_place_infections) 
	   		to: "captured_data360.csv" type: "csv" ;
	   	
	   	}
	
	}
	
	reflex stop_simulation when: (days_passed = 365) {
		write "dadadd" ;
		do halt ;
	} 
		...
		if flip(1 - fetal_mortality){
			if(flip(1 - mortality_emergence)){
					if(age>=max_age){
						...
						//		if(myself.is_infected = true) {write "infected mosquito.....";}
								mos_age <- 0;
							}
...
species mosquito skills:[moving]{	
	...
	reflex reproduce when: time_since_eggs>=3 and time mod 86400 = 0{
		...
		//	write("eggreproduced" + eggreproduced);
			create egg number:eggreproduced{
				living_place <- myself.living_place ;
				age <- 0;
				max_age <- 5;
				is_infected <-myself.is_infected;
			//	if(myself.is_infected = true){write("infected egg");}
			}		
		time_since_eggs <- 0;
		carrying_eggs <- false;
		//
